...
class DataHandler:
    ...
    def get_data_files(self):
        predir = os.path.join(args.data_root, self.data_name)
        if not os.path.isdir(predir):
            raise FileNotFoundError(
                f'Dataset directory not found: {predir}. '
                f'Please set --data_root to the folder containing "{self.data_name}".'
            )
        if os.path.exists(os.path.join(predir, 'feats.pkl')):
            self.feat_file = os.path.join(predir, 'feats.pkl')
        else:
            self.feat_file = None
        self.trnfile = os.path.join(predir, 'trn_mat.pkl')
        self.tstfile = os.path.join(predir, 'tst_mat.pkl')
        self.fewshotfile = os.path.join(predir, 'partial_mat_{shot}.pkl'.format(shot=args.ratio_fewshot))
        self.valfile = os.path.join(predir, 'val_mat.pkl')
        self.edge_eval_payload_file = os.path.join(predir, args.edge_eval_payload_name)
    ...
    def load_feats(self, filename):
        try:
            ...
            raise RuntimeError(f'Failed to load feats file: {filename}') from exc
        return feats

    def load_torch_payload(self, filename):
        try:
            return t.load(filename, map_location='cpu', weights_only=False)
        except TypeError:
            return t.load(filename, map_location='cpu')

    def load_edge_eval_payload(self, filename):
        if not os.path.exists(filename):
            if str(getattr(args, 'eval_protocol', 'agae')).lower() == 'agae':
                raise FileNotFoundError(
                    f'Edge eval payload missing: {filename}. '
                    'AGAE eval protocol requires this file.'
                )
            return None
        payload = self.load_torch_payload(filename)
        if not isinstance(payload, dict):
            raise ValueError(f'Invalid edge eval payload: {filename}')

        def _as_edge_index(key):
            edge_index = payload.get(key)
            if edge_index is None:
                raise ValueError(f'Missing key in edge eval payload: {key} ({filename})')
            edge_index = t.as_tensor(edge_index, dtype=t.long)
            if edge_index.numel() == 0:
                return t.empty((2, 0), dtype=t.long)
            if edge_index.dim() != 2 or edge_index.size(0) != 2:
                raise ValueError(f'Invalid edge index tensor for key={key} in {filename}')
            return edge_index

        return {
            'val_pos_edge_index': _as_edge_index('val_pos_edge_index'),
            'val_neg_edge_index': _as_edge_index('val_neg_edge_index'),
            'test_pos_edge_index': _as_edge_index('test_pos_edge_index'),
            'test_neg_edge_index': _as_edge_index('test_neg_edge_index'),
    ...
    def load_data(self):
        ...
        self.edge_eval_payload = self.load_edge_eval_payload(self.edge_eval_payload_file)
