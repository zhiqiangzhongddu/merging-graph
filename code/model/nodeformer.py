    ...
    if is_query:
        ...
        data_dash = ratio * (
            torch.exp(
                ...
                - diag_data
                - torch.max(
                    torch.max(data_dash, dim=last_dims_t, keepdim=True)[0], dim=attention_dims_t, keepdim=True
                )[0]
            )
            + numerical_stabilizer
        )
...
def kernelized_gumbel_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None, K=10, tau=0.25, return_weight=True):
    """
    Fast computation of all-pair attentive aggregation with linear complexity with Gumbel noise.
    """
    query = query / math.sqrt(tau)
    key = key / math.sqrt(tau)
    query_prime = kernel_transformation(query, True, projection_matrix)  # [B, N, H, M]
    ...
    def forward(self, x, adjs, tau=1.0):
        ...
        for i, conv in enumerate(self.convs):
            if self.use_edge_loss:
                ...
                link_loss_append = getattr(link_loss_, "append")
                link_loss_append(link_loss)
            else:
                z = conv(z, adjs, tau)
...
class NodeFormerEncoder(nn.Module):
    """
    Project wrapper: returns (node_repr, graph_repr) for downstream tasks.
    """

    def __init__(self, cfg, in_dim: int):
        super().__init__()
        ncfg = getattr(cfg.model, "nodeformer", None)
        heads = getattr(ncfg, "heads", 4)
        nb_random_features = getattr(ncfg, "num_random_features", 30)
        tau = getattr(ncfg, "tau", 1.0)
        dropout = getattr(cfg.model, "dropout", 0.1)
        self.tau = tau
        self.use_edge_loss = getattr(ncfg, "use_edge_loss", False)
        self.model = NodeFormer(
            in_channels=in_dim,
            ...
            dropout=dropout,
            nb_random_features=nb_random_features,
            use_bn=getattr(ncfg, "use_layernorm", True),
            use_gumbel=getattr(ncfg, "use_gumbel", True),
            use_residual=getattr(ncfg, "use_residual", True),
            use_act=getattr(ncfg, "use_activation", True),
            use_jk=getattr(ncfg, "use_jk", False),
            nb_gumbel_sample=getattr(ncfg, "num_gumbel_samples", 10),
            rb_order=getattr(ncfg, "rb_order", 0),
            rb_trans=getattr(ncfg, "rb_trans", "sigmoid"),
